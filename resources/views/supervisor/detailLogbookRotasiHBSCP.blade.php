...
@section('title', 'Detail Logbook Rotasi HBSCP')
...
<div class="mx-auto p-0 sm:p-6 min-h-screen pt-5 lg:pt-20">
    <div class="bg-white shadow-xl rounded-none sm:rounded-2xl overflow-hidden border-0 sm:border border-gray-100">
        {{-- Header --}}
        <div class="bg-gradient-to-r from-teal-500 to-cyan-600 px-4 sm:px-6 py-4 sm:py-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-xl sm:text-2xl font-bold">Detail Logbook Rotasi HBSCP</h3>
                    <p class="text-teal-100 text-sm sm:text-base">ID Logbook: {{ $logbook->id }}</p>
                </div>
                <a href="{{ route('supervisor.logbook-rotasi.list', ['type' => 'hbscp']) }}" class="hidden sm:inline-flex items-center px-4 py-2 bg-white bg-opacity-20 text-white rounded-lg hover:bg-opacity-30 transition-colors font-medium text-sm">
        ...
        <div class="p-4 sm:p-6 space-y-6">
            ...
            @include('logbook.rotasi.partials.tabel_hbscp', ['officerLog' => $officerLog, 'logbook' => $logbook])
         ...
         <div class="mt-6 px-4 sm:px-0">
            <a href="{{ route('supervisor.logbook-rotasi.list', ['type' => 'hbscp']) }}" class="inline-flex sm:hidden items-center px-4 py-2 bg-white text-gray-700 rounded-lg border hover:bg-gray-100 font-medium text-sm w-full justify-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                Kembali ke Daftar
            </a>
        </div>
