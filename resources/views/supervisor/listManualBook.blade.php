...
<div class="max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-6 lg:pt-20">
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-3 sm:px-6 py-4 sm:py-5">
                <div
                    ...
                    <div>
                        <h2 class="text-lg sm:text-xl font-bold text-white">Buku Pemeriksaan Manual HBSCP</h2>
                        <p class="text-blue-100 text-xs sm:text-sm mt-1">
                            {{ $manualBooksHBSCP->total() }} data ditemukan
                            @if(request()->has('status') && request()->get('status') !== '')
                            (difilter berdasarkan status: {{ ucfirst(request()->get('status')) }})
                            @endif
                        </p>
                    </div>
                    <div
                        class="bg-blue-500 text-white px-3 py-1 sm:px-4 sm:py-2 rounded-full font-semibold text-xs sm:text-sm">
                        {{ $manualBooksHBSCP->count() }} dari {{ $manualBooksHBSCP->total() }} Data
                    </div>
                </div>
            </div>
            ...
            <div class="block lg:hidden p-2 sm:p-4 space-y-3">
                ...
                <div class="text-center py-8">
                    ...
                    <h3 class="text-base sm:text-lg font-semibold text-gray-500 mb-1">Tidak Ada Data Ditemukan</h3>
                    <p class="text-xs sm:text-sm text-gray-400">
                        @if(request()->has('status') && request()->get('status') !== '')
                        Tidak ada data HBSCP dengan status "{{ ucfirst(request()->get('status')) }}"
                        @else
                        Belum ada data HBSCP yang tersedia
                        @endif
                    </p>
                </div>
                @endforelse
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-8 sm:py-12 text-center">
                                ...
                                <h3 class="text-base sm:text-lg font-semibold text-gray-500 mb-1">Tidak Ada Data
                                    Ditemukan</h3>
                                <p class="text-xs sm:text-sm text-gray-400">
                                    @if(request()->has('status') && request()->get('status') !== '')
                                    Tidak ada data HBSCP dengan status "{{ ucfirst(request()->get('status')) }}"
            ...
            <div class="mt-4 sm:mt-6 px-2 sm:px-4 pb-4 flex justify-center">
                {{ $manualBooksHBSCP->appends(request()->except('page'))->links() }}
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="mb-8 sm:mb-10">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
            {{-- Header --}}
            <div class="bg-gradient-to-r from-green-600 to-green-700 px-3 sm:px-6 py-4 sm:py-5">
                <div
                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0">
                    <div>
                        <h2 class="text-lg sm:text-xl font-bold text-white">Buku Pemeriksaan Manual PSCP</h2>
                        <p class="text-green-100 text-xs sm:text-sm mt-1">
                            {{ $manualBooksPSCP->total() }} data ditemukan
                            @if(request()->has('status') && request()->get('status') !== '')
                            (difilter berdasarkan status: {{ ucfirst(request()->get('status')) }})
                            @endif
                        </p>
                    ...
                    <div
                        ...
                        {{ $manualBooksPSCP->count() }} dari {{ $manualBooksPSCP->total() }} Data
                    </div>
                </div>
            </div>
            ...
            <div class="block lg:hidden p-2 sm:p-4 space-y-3">
                ...
                <div
                    class="border border-gray-200 rounded-xl p-3 sm:p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-start mb-3">
                        <div class="bg-green-100 text-green-800 px-2 py-1 rounded-full font-semibold text-xs">
                            ID: {{ $PSCP->id }}
                        </div>
                        <div class="text-right">
                    ...
                    <div class="pt-2 border-t border-gray-100">
                        <a href="{{ route('supervisor.checklist-manualbook.detail', $PSCP->id) }}"
                            class="w-full inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors duration-200 font-semibold text-sm shadow-sm">
                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path>
                                <path fill-rule="evenodd"
                                    d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z"
                ...
                <div class="text-center py-8">
                    ...
                    <p class="text-xs sm:text-sm text-gray-400">
                        @if(request()->has('status') && request()->get('status') !== '')
                        Tidak ada data PSCP dengan status "{{ ucfirst(request()->get('status')) }}"
                        @else
                        Belum ada data PSCP yang tersedia
                        @endif
                    </p>
                </div>
                @endforelse
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="min-w-full">
                    <thead class="bg-green-50 border-b border-green-200">
                        <tr>
                            <th
                                class="px-3 sm:px-4 py-3 sm:py-4 text-left text-xs sm:text-sm font-semibold text-green-800">
                                <div class="flex items-center">
                                    <svg class="w-3 sm:w-4 h-3 sm:h-4 mr-1 sm:mr-2" fill="currentColor"
                                        viewBox="0 0 20 20">
                                        <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"></path>
                            ...
                            <th
                                class="px-3 sm:px-4 py-3 sm:py-4 text-center text-xs sm:text-sm font-semibold text-green-800">
                                <div class="flex items-center justify-center">
                                    <svg class="w-3 sm:w-4 h-3 sm:h-4 mr-1 sm:mr-2" fill="currentColor"
                                        viewBox="0 0 20 20">
                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-green-50 transition-colors duration-200">
                            <td class="px-3 sm:px-4 py-3 sm:py-4">
                                <span
                                    class="bg-green-100 text-green-800 px-2 sm:px-3 py-1 rounded-full font-semibold text-xs sm:text-sm">
                                    {{ $PSCP->id }}
                                </span>
                            </td>
                            <td class="px-3 sm:px-4 py-3 sm:py-4">
                            ...
                            <td class="px-3 sm:px-4 py-3 sm:py-4 text-center">
                                <a href="{{ route('supervisor.checklist-manualbook.detail', $PSCP->id) }}"
                                    class="inline-flex items-center px-3 sm:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors duration-200 font-semibold text-xs sm:text-sm shadow-sm">
                                    <svg class="w-2 sm:w-3 h-2 sm:h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path>
                                        <path fill-rule="evenodd"
                                            d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z"
            ...
            <div class="mt-4 sm:mt-6 px-2 sm:px-4 pb-4 flex justify-center">
                {{ $manualBooksPSCP->appends(request()->except('page'))->links() }}
            </div>
            @endif
        </div>
    </div>
