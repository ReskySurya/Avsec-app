...
<div x-data="{
        ...
        <div class="bg-gradient-to-r from-blue-500 to-teal-600 px-6 py-6 text-white">
            <div class="flex flex-col sm:flex-row justify-between items-start">
                <div>
                    ...
                    <p class="text-blue-100">Catatan aktivitas harian Buku Pemeriksaan Manual</p>
                </div>
                <button @click="openManualBook = true"
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200"
                ...
                <div class="bg-gray-50 px-5 py-3 flex justify-end space-x-2">
                    <button type="button"
                        ...
                        @click.stop="
                            openEditManualBook = true;
                            editManualBookData.logbookID = '{{ $logbook->logbookID }}';
                            editManualBookData.date = '{{ $logbook->date->format('Y-m-d') }}';
                            editManualBookData.grup = '{{ addslashes($logbook->grup ?? '') }}';
                            editManualBookData.shift = '{{ addslashes($logbook->shift ?? '') }}';
                        ">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap flex space-x-2">
                            <button type="button"
                                ...
                                @click.stop="
                                    ...
                                    editManualBookData.logbookID = '{{ $logbook->logbookID }}';
                                    editManualBookData.date = '{{ $logbook->date->format('Y-m-d') }}';
                                    editManualBookData.location_area_id = '{{ $logbook->location_area_id }}';
                                    editManualBookData.grup = '{{ addslashes($logbook->grup ?? '') }}';
                                    editManualBookData.shift = '{{ addslashes($logbook->shift ?? '') }}';
                                ">
    ...
    <div x-show="openManualBook" x-transition
        ...
        <div @click.away="openManualBook = false" x-data="{ selectedRotasi: '', selectedTempatJaga: '' }"
            ...
            <div class="bg-gradient-to-r from-blue-500 to-teal-600 text-white p-6 rounded-t-2xl">
                <h2 class="text-2xl font-bold">Tambah Data Buku Pemeriksaan Manual</h2>
                <p class="text-blue-100">Tambahkan catatan Buku Pemeriksaan Manual</p>
            </div>
            ...
            <form action="{{ route('logbookRotasi.store') }}" method="POST" class="p-6">
                ...
                {{-- 2. Input Shift (Tetap) --}}
                <div class="mb-4">
                    <label for="shfit" class="block text-sm font-semibold text-gray-700 mb-2">Dinas/Shift</label>
                    <select id="shfit" name="shfit" required
                        class="w-full border-2 border-gray-200 px-4 py-3 rounded-xl focus:border-blue-500 focus:outline-none transition-colors duration-200">
                ...
                {{-- 3. Dropdown Rotasi (Trigger) --}}
                <div class="mb-4">
                    <label for="type" class="block text-sm font-semibold text-gray-700 mb-2">Area Jaga</label>
    ...
    {{-- Modal Edit Logbook --}}
    <div x-show="openEditManualBook" x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100"
        x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform scale-100"
        x-transition:leave-end="opacity-0 transform scale-95"
        class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" style="display: none;">
        <div @click.away="openEditManualBook = false" class="bg-white w-full max-w-md rounded-2xl shadow-2xl">
            <div class="bg-gradient-to-r from-blue-500 to-teal-600 text-white p-6 rounded-t-2xl">
                <h2 class="text-2xl font-bold">Edit Entry Logbook</h2>
                <p class="text-blue-100">Ubah informasi logbook</p>
            </div>
            <form :action="'{{ url('/logbook/posjaga') }}/' + editManualBookData.logbookID" method="POST" class="p-6">
                @csrf
                @method('PATCH')
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal</label>
                    <input type="date" name="date" required x-model="editManualBookData.date"
                        class="w-full border-2 border-gray-200 px-4 py-3 rounded-xl focus:border-blue-500 focus:outline-none transition-colors duration-200">
                    @error('date')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <select id="edit_location_area_id" name="location_area_id" required
                        x-model="editManualBookData.location_area_id"
                        class="w-full border-2 border-gray-200 px-4 py-3 rounded-xl focus:border-blue-500 focus:outline-none transition-colors duration-200">
                ...
                <div class="mb-6">
                    ...
                    <select name="grup" required x-model="editManualBookData.grup"
                        class="w-full border-2 border-gray-200 px-4 py-3 rounded-xl focus:border-blue-500 focus:outline-none transition-colors duration-200">
                ...
                <div class="mb-6">
                    ...
                    <select name="shift" required x-model="editManualBookData.shift"
                        class="w-full border-2 border-gray-200 px-4 py-3 rounded-xl focus:border-blue-500 focus:outline-none transition-colors duration-200">
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button" @click="openEditManualBook = false"
                        class="px-6 py-3 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition-colors duration-200 font-medium">
                        Batal
                    </button>
                    <button type="submit"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const closeButtons = document.querySelectorAll('[x-on\\:click="openManualBook = false"]');
