...
<div class="mx-auto p-0 sm:p-6 min-h-screen pt-5 sm:pt-20">
    <div class="flex items-center justify-between mb-8">
        <a href="{{ route('history.index') }}" class="inline-flex items-center text-gray-600 hover:text-indigo-600 transition-colors duration-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
            </svg>
            <span>Kembali ke Kategori</span>
        </a>
        <h3 class="text-lg sm:text-xl font-bold text-gray-800 leading-tight text-right">Riwayat: {{ ucwords(str_replace('-', ' ', $category)) }}</h3>
    </div>
    ...
    <div class="bg-white shadow-lg rounded-xl p-6 mb-8">
        ...
        <form action="{{ route('history.show', ['category' => $category]) }}" method="GET">
                ...
                <div>
                    ...
                    <select name="form_type" id="form_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        @foreach($form_types as $type)
                            <option value="{{ $type }}" {{ request('form_type') == $type ? 'selected' : '' }}>{{ ucwords(str_replace('_', ' ', $type)) }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mt-6 text-right">
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
    ...
    <div class="space-y-8">
        @forelse ($data as $key => $items)
            @php
                ...
                $groupTitle = $isGrouped ? ucwords(str_replace('_', ' ', $key)) : (request('form_type') ? ucwords(str_replace('_', ' ', request('form_type'))) : 'Hasil Pencarian');
            @endphp
            ...
            <div class="bg-white shadow-lg rounded-xl">
                ...
                <div class="divide-y divide-gray-200">
                    ...
                    <div class="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between hover:bg-gray-50 transition-colors duration-200">
                        <div class="flex-grow mb-4 sm:mb-0">
                            <div class="flex items-center mb-2 flex-wrap">
                                <span class="font-bold text-indigo-600 text-md">{{ ucwords(str_replace('_', ' ', $item->form_type)) }}</span>
                            ...
                            <div class="text-sm text-gray-600 mt-1 flex items-center flex-wrap">
                                <span><span class="font-medium text-gray-800">Tanggal:</span> {{ \Carbon\Carbon::parse($item->date)->isoFormat('D MMMM YYYY') }}</span>
                                <span class="mx-2 text-gray-300 sm:inline">|</span>
                                <span><span class="font-medium text-gray-800">Shift:</span> {{ ucwords($item->shift ?? 'N/A') }}</span>
                                @php
                        ...
                        <div class="flex-shrink-0 self-end sm:self-center">
                            <a href="{{ route('history.preview', ['category' => $category, 'id' => $item->id, 'form_type' => $item->form_type]) }}" target="_blank" class="inline-flex items-center p-2 bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200 transition-colors duration-300" title="Lihat Pratinjau">
        ...
        @empty
            <div class="bg-white shadow-lg rounded-xl p-8 text-center">
                <div class="flex flex-col items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-gray-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
