{{-- File: resources/views/logbook/rotasi/partials/tabel_pscp.blade.php --}}
        ...
        <table class="min-w-full text-sm text-center">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-3">@foreach($roleData as $slot) {!! $slot['start'] !!}<br> @endforeach</td>
                    <td class="p-3">@foreach($roleData as $slot) {!! $slot['end'] !!}<br> @endforeach</td>
                    <td class="p-3 font-semibold text-blue-600">@foreach($roleData as $slot) {{ $slot['hhmd_random'] ?? '-' }}<br> @endforeach</td>
                    <td class="p-3 font-semibold text-blue-600">@foreach($roleData as $slot) {{ $slot['hhmd_unpredictable'] ?? '-' }}<br> @endforeach</td>

                    {{-- Manual Kabin (dengan Counter) --}}
                    @php $roleData = $data['roles']['manual_kabin_petugas'] ?? []; @endphp
                    <td class="p-3">@foreach($roleData as $slot) {!! $slot['start'] !!}<br> @endforeach</td>
                    <td class="p-3">@foreach($roleData as $slot) {!! $slot['end'] !!}<br> @endforeach</td>
                    <td class="p-3 font-semibold text-green-600">@foreach($roleData as $slot) {{ $slot['cek_random_barang'] ?? '-' }}<br> @endforeach</td>
                    <td class="p-3 font-semibold text-green-600">@foreach($roleData as $slot) {{ $slot['barang_unpredictable'] ?? '-' }}<br> @endforeach</td>

                    <td class="p-3 text-left text-xs">{!! implode('<br>', array_unique(array_filter($data['keterangan']))) !!}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="18" class="text-center text-gray-500 p-6">Tidak ada data entri PSCP untuk ditampilkan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="p-4 space-y-4">
                ...
                <div class="bg-blue-100 rounded-lg p-3 mb-2">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm pt-3 border-t border-blue-200">
                        <div class="text-center">
                            <span class="text-gray-600 text-xs block">HHMD Random</span>
                            <div class="text-xl font-bold text-blue-600">{{ $slot['hhmd_random'] ?? '0' }}</div>
                        </div>
                        <div class="text-center">
                            <span class="text-gray-600 text-xs block">HHMD Unpredictable</span>
                            <div class="text-xl font-bold text-blue-600">{{ $slot['hhmd_unpredictable'] ?? '0' }}</div>
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="border-l-4 border-green-600 pl-4">
                ...
                <div class="bg-green-100 rounded-lg p-3 mb-2">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm pt-3 border-t border-green-200">
                        <div class="text-center">
                            <span class="text-gray-600 text-xs block">Cek Random</span>
                            <div class="text-xl font-bold text-green-600">{{ $slot['cek_random_barang'] ?? '0' }}</div>
                        </div>
                        <div class="text-center">
                            <span class="text-gray-600 text-xs block">Unpredictable</span>
                            <div class="text-xl font-bold text-green-600">{{ $slot['barang_unpredictable'] ?? '0' }}</div>
                        </div>
                    </div>
                </div>
                @endforeach
